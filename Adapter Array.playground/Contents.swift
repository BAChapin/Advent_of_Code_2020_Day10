...
struct Connections {
    ...
    init(_ list: [String]) {
        self.ratings = list.compactMap { Int($0) }
        self.ratings.append(0)
        ratings.sort()
        self.ratings.append(ratings[ratings.count - 1] + 3)
    }
    
    func joltDifferences() -> (one: Int, two: Int, three: Int) {
        var counts: [Int] = [0, 0, 0]
        
        for index in 1..<ratings.count {
            let num = ratings[index - 1]
            let num2 = ratings[index]
...
func answerForPartOne() {
    let connections = Connections(list)
    let differences = connections.joltDifferences()
...
/*
 ...
 Success: 86812553324672
 
 */

func answerForPartTwo() {
    let connections = Connections(list)
    let permeations = connections.permeatations()
